...
void sort_rects(vector <Rect> object_rects, vector<Rect>& sorted_rects)
{
	...
	Rect temp = object_rects[pos_x.at<int>(0, 0)];
	for (int i = 1; i < pos_x.rows; i++)	{
		int idx = pos_x.at<int>(i, 0);
		if (abs(temp.x - object_rects[idx].x)<2) {
			if (temp.height <= temp.width) {
				if (temp.y < object_rects[idx].y)
					temp = Rect(temp.br(), object_rects[idx].tl());
				else
					temp = Rect(object_rects[idx].tl(), temp.br());
			}
			else {
				sorted_rects.push_back(temp);
				temp = object_rects[idx];
			}
		}
		else {
			if(temp.height >= temp.width)
...
void classify_numbers(vector<Mat> numbers, Ptr<ml::SVM> svm, Ptr<ml::KNearest> knn[1], int K1)
{
	...
	for (int i = 0; i < (int)numbers.size(); i++)
	{
		...
		Mat results;
		if (i == 2) {
			knn[0]->findNearest(data, K1, results);				// 숫자 k-NN 분류 수행
			cout << text_value[(int)results.at<float>(0)] << " ";// 결과 출력
		}
		else {
			//cvtColor(data, data, CV_GRAY2BGR);
			cout << classify_number(svm, data) << " ";
